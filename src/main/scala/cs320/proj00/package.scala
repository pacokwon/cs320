...
package object proj00 extends Project00 {
  ...
  def recHelper(ds: List[FunDef], b: Expr, env: Env): Value = {
    val envWithFuncs = ds.foldLeft(env)((envAcc, definition) => {
      val function = CloV(definition.ps, definition.b, envAcc)
      function.env = envAcc + (definition.n -> function)
      envAcc + (definition.n -> function)
    })

    interp(b, envWithFuncs)
  }
  ...
  def interp(exp: Expr, env: Env): Value =
    exp match {
      ...
      case RecFuns(ds, b) => recHelper(ds, b, env)
    }
